...
@pytest.mark.asyncio
async def test_check_budget_reports_tokens_and_warning_threshold(db_session, monkeypatch):
    from needradar.core.config import settings

    monkeypatch.setattr(settings, "budget_daily_limit", 0.01)
    monkeypatch.setattr(settings, "budget_monthly_limit", 0.0)
    monkeypatch.setattr(settings, "budget_alert_threshold_percent", 80.0)
    svc = UsageService(db_session)
    await svc.record_usage(
        preset_id="dp",
        model="v4",
    ...
    assert result["daily"]["percent_used"] == 80.0
    assert result["alerts"] == [
        {
            "level": "daily",
            "severity": "warning",
            ...
            "spent": 0.008,
            "percent_used": 80.0,
            "message": "daily预算接近上限：¥0.0080 / ¥0.0100",
        }
    ]


@pytest.mark.asyncio
async def test_check_budget_reports_exceeded_threshold(db_session, monkeypatch):
    from needradar.core.config import settings
    ...
    assert result["alerts"][0]["severity"] == "exceeded"
