...
NeedRadar 是一个 AI 驱动的用户需求挖掘系统。从全网（GitHub、StackOverflow、掘金等）爬取讨论，通过 LLM 提取结构化需求，存储到 Obsidian vault，生成洞察报告并进行内容验证。

技术栈：Python / FastAPI / SQLAlchemy / SQLite / Vue 3 / Naive UI / LiteLLM / DeepSeek / ChromaDB
...
src/needradar/
  api/v1/                 # REST API 路由
  crawlers/               # 各平台爬虫
  llm/                    # LLM 调用层（provider, pricing, presets）
  models/                 # 数据模型
  services/               # 业务逻辑
  vector/                 # ChromaDB 向量存储（去重）
